...
Plotpca <- function(Data, label, title, Datatype = 'scRNA-seq'){
  pca <- prcomp(t(Data))
  Plot(pca$x[, 1:2], as.character(label), title, 'pca 1', 'pca 2', paste0(Datatype, '_pca.pdf'))
}
...
Plotumap <- function(Data, label, Datatype = 'scRNA-seq', need_PCA = TRUE){
  ...
  Plot(umap, as.character(label), paste0('Umap for ', Datatype), 'umap 1', 'umap 2', paste0(Datatype, '_umap.pdf'))
}
